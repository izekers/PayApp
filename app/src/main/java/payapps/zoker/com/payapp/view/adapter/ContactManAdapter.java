 ...
 */
public class ContactManAdapter extends RecyclerView.Adapter<VisitableViewHolder> {
    private VisitableTypeControl.TypeFactory typeFactory;
    private List<ContactMan> mDatas;
    public ContactManAdapter(VisitableTypeControl.TypeFactory typeFactory) {
    ...
    public boolean setDel(boolean isShow){
        if (mDatas==null)
            return false;
        for (ContactMan contactMan:mDatas){
            contactMan.setShowDel(isShow);
        }
        notifyDataSetChanged();
        return isShow;
    }
